...
from pyworkflow.em.packages.xmipp3.protocol_preprocess.geometrical_mask import *
...
class XmippProtCreateMask3DFluo(ProtCreateMask3D, XmippGeometricalMask3D):
    """ Create a 3D mask.
    The mask can be created with a given geometrical shape (Sphere, Box, Cylinder...) or
    from operating on a 3d volume or a previuous mask or
    from expanding a 2D mask in the z direction.
    """
    _label = 'create 3d mask fluo'
    
    #--------------------------- DEFINE param functions --------------------------------------------
    def _defineParams(self, form):
        form.addSection(label='Mask generation')
        form.addParam('source', EnumParam, default=SOURCE_VOLUME,
                      choices=['Volume','Geometry' ,'Input 2D mask'],
                      label='Mask source')
        # For volume sources
        ...
        # For input 2D mask
        form.addParam('input2DMask', PointerParam, pointerClass='Mask', 
                      condition='source==%d' % SOURCE_INPUT2D,
                      label="Input 2D mask",
                      help="Select the input 2D mask")
#         form.addParam('input2DMask', PointerParam, pointerClass="Micrograph", 
#                       condition='source==%d' % SOURCE_INPUT2D, 
#                       label="Input 2D mask",
#                       help="Select the input 2D mask")
        form.addParam('zsize', FloatParam, default=10.0,
                      condition='source==%d' % SOURCE_INPUT2D, 
                      label='Z size (pixel)')
        form.addParam('z0', FloatParam, default=1.0,
                      condition='source==%d' % SOURCE_INPUT2D, 
                      label='min Z mask coord  (pixel)')
        form.addParam('z1', FloatParam, default=8.0,
                      condition='source==%d' % SOURCE_INPUT2D, 
                      label='max Z mask coord (pixel)')
    ...
    def _insertAllSteps(self):
        ...
        elif self.source == SOURCE_INPUT2D:
            self.inputVolume.set(None)
            self._insertFunctionStep('createMaskFromInput2DMaskStep')
    ...
    def createMaskFromInput2DMaskStep(self):
        Mask2D = self.input2DMask.get()
        fnMask2D = getImageLocation(Mask2D)
        # Create empty volume file with desired dimensions
#         mdim = Mask2D.getDimensions()
#         xmipp.createEmptyFile(self.maskFile, mdim[1], mdim[2], self.zsize.get())

        # Create the mask
        args = '-i %s ' % fnMask2D
        args += '-zdim %d ' % (float)(self.zsize)
        args += '-z0 %d ' % (float)(self.z0)
        args += '-z1 %d ' % (float)(self.z1)
        args += ' -o %s' % self.maskFile
        self.runJob("xmipp_create_mask3D_from_mask2D", args)
        return [self.maskFile]
