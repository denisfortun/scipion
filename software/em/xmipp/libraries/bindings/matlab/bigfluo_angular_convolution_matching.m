            ...
            for shiftX=shift_init:shift_sampling:shift_end      
               for shiftY=shift_init:shift_sampling:shift_end      
                    for shiftZ=shift_init:shift_sampling:shift_end      
                        count = count + 1;
%                         fprintf('pose nb %d\n',count);
                        transConvRotRecon = imtranslate(convRotRecon,[shiftX,shiftY,shiftZ]);
                        figure(1);imagesc(squeeze(transConvRotRecon(:,:,floor(size(transConvRotRecon,3)/2)))); axis image ; colormap gray;
                        figure(2);imagesc(squeeze(transConvRotRecon(floor(size(transConvRotRecon,1)/2),:,:))); axis image ; colormap gray;    
                        %% Matching
                        for i_vol=1:nbPoses
